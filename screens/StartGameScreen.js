...
const StartGameScreen = ({ onStartGame }) => {                   // App.js'den aldığımız "onStartGame" prop'unu aşağıdaki "Start Game" butonunun olduğu kısımda kullanıcaz
    ...
    const confirmInputHandler = () => {
        ...
        if (chosenNumber === NaN || chosenNumber <= 0 || chosenNumber > 99) {
            Alert.alert("It's an unvalid number!", "Number has to be between 1 and 99", [{ text: "Okay", style: "destructive", onPress: resetInputHandler }]);
            return;
        }
    ...
    return (
        <TouchableWithoutFeedback onPress={() => {             // Ekranda herhangi bir yere tıkladığımız zaman
